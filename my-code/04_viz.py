...
drinks.groupby('continent').spirit_servings.mean().plot(kind='bar', title='Average Number of Beer Servings By Continent')
...
# by continent.
drinks.groupby('continent').total_litres_of_pure_alcohol.mean().plot(kind='bar', title='Avg. total litres of pure alcohol by continent')
plt.show()

# 2. Illustrate the relationship between spirit servings and total litres of 
# pure alcohol.  What kind of relationship is there?
drinks.plot(kind='scatter', x='spirit_servings', y='total_litres_of_pure_alcohol', alpha=0.5)
plt.title('Spirit Servings v. Total Litres of Pure Alcohol')
...
plt.suptitle('Spirit servings by continent')
plt.xlabel('spirit servings')
plt.show()
